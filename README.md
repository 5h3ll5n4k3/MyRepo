# 👨‍💻 Cybersecurity Professional | Offensive Security Builder  

Hey 👋 I’m a dedicated Multidisciplinary Operator with demonstrated proficiency in both offensive and defensive domains, leveraging hands-on experience who thrives on building, breaking, and automating. My repos focus on practical security tools, red team tactics, and labs for sharpening offensive and defensive tradecraft.  

I like creating **realistic attack simulations** and **automation scripts** that help people learn how attackers operate — and how defenders can spot them.  

---

## 🔒 Focus Areas

- 🔹 Red Team Operations & Physical Security Testing
- 🔹 PowerShell & Python for Automation & Payloads
- 🔹 HID-based Attack Simulation (e.g., USB Rubber Ducky)
- 🔹 Endpoint Evasion Techniques (for education & defense)
- 🔹 Adversary Emulation Labs (Atomic Red Team-style)

---

## 🛠 Projects & Labs

I like to build and publish:

- ⚙️ Scripted payloads to demonstrate attack paths
- 📂 GitHub repos for Ducky Script, PowerShell, and red team tools
- 💡 Small utilities that simplify reconnaissance or evasion
- 🧪 Labs for endpoint response, detection, or training

Most of these are **educational** and built for **authorized environments only**.

---

## 🌐 Connect With Me

- 💼 [LinkedIn](https://www.linkedin.com/) *(add your link)*
- 🧠 [TryHackMe](https://tryhackme.com/) / [HackTheBox](https://www.hackthebox.com/) *(add your handles)*
- 📧 [Email Me](mailto:you@example.com) for collaboration or ideas

---

## 🧰 Tools I Use Often

- PowerShell / Python / Bash
- Kali Linux / Windows VMs
- Rubber Ducky / Flipper Zero / Wifi Pinapple
- NetExec / Nmap / Hydra
- Windows Event Logs & Sysmon for detection engineering

---

Thanks for stopping by — feel free to explore my repositories, drop feedback, or suggest collaboration ideas!

---

📜 *All code shared here is for educational, authorized, and ethical use only.*
